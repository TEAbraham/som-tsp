...
def plot_network(cities, neurons, name='diagram.png', ax=None):
    ...
    if not ax:
        ...
        axis.scatter(cities['x'], cities['y'], color='red', s=4, zorder=-1)
        ...
        ax.scatter(cities['x'], cities['y'], color='red', s=4, zorder=-1)
...
def plot_route(cities, route, name='diagram.png', ax=None):
    ...
    if not ax:
        ...
        axis.plot(route['x'], route['y'], color='purple', linewidth=1, zorder=1)
        ...
        route = cities.reindex(route)
        route.loc[route.shape[0]] = route.iloc[0]
        ax.plot(route['x'], route['y'], color='purple', linewidth=1, zorder=1)
        return ax
